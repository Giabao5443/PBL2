...
void List::Them_Thong_Tin()
{
	...
	KtMsv:
	{
		cout << "\nYeu cau MSV co 9 ki tu." << endl;
		cout << " Nhap ma sinh vien: ";
		
		getline(cin, msv);
		}
		if (msv.length() != 9) goto KtMsv;
		else{
		for (int i = 0; i < size;i++){
			if(msv == sv[i].GetMsv())
...
bool List::TimKiemTuanTu()
{
	...
	TuyChon:
		...
		case 2:
		{
			...
			for (int i = 0; i < size; i++)
			{
				if (ngaysinhtimkiem ==sv[i].GetNgaySinh())
				{
					sv[i].XuatSinhVien();
					check++;
				}
	...
	TuyChon:
		...
		switch (chon)
		{
			...
			case 1:
			{
				...
				for (int i = 0; i < size; i++)
				{
					BinarySearchHoTen(0, size - 1, hoten, sv[i].GetHoten());
				}
				if (check == 0)
					cout << "\nKhong co thong tin can tim";
				cout << "\nBan co muon thoat ra khong?(y/n)";
			...
			case 5:
			{
				...
				for (int i = 0; i < size; i++)
				{
					BinarySearchDiem(0, size - 2, diemtimkiem, sv[i].GetDiem());					
				}
				if (check == 0) cout << "\nKhong co thong tin can tim";
				cout << "\nBan co muon thoat ra khong?(y/n)";
				char checkexit;
...
int List::BinarySearchHoTen(int r, int l, string x, string timkiemHoTen)
{
	...
	if (r >= l)
	{
		...
		// Nếu arr[mid] > x, thực hiện tìm kiếm nửa trái của mảng
		if (timkiemHoTen > x)
			return BinarySearchHoTen(l, mid - 1, x, timkiemHoTen);

		// Nếu arr[mid] < x, thực hiện tìm kiếm nửa phải của mảng
		return BinarySearchHoTen(mid + 1, r, x, timkiemHoTen);
	}
